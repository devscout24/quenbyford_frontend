...
const WeeklyView = ({ currentWeekStart, currentWeekEnd, days, events }: WeeklyViewProps) => {
  ...
  return (
    <div className="max-w-[850px] mx-auto rounded-xl overflow-hidden border border-slate-200 shadow-xl">
      {/* Orange Header */}
      ...
      <div className="grid grid-cols-7 bg-[#F97316] ">
        ...
        {weeklyDays.map((day, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`min-h-25 relative transition-colors border-x border-gray-300 
            ...
            >
              ...
              {dayEvents.map((evt, eIdx) => (
                <div key={eIdx} className={`mt-2 text-center rounded px-2 py-1 ${isToday ? '' : ''}`}>
                  <span className={`text-xs font-bold leading-tight block ${isToday ? 'text-white' : 'text-orange-600'}`}>
                    {evt.title}
                  </span>
                </div>
              ))}
