...
const AttendExpoSection = () => {
  ...
  const dayData = [
    {
      day: t("events.attend.day1_title"),
      bg: "bg-[#F97316]/20",
      content: t("events.attend.day1_desc")
    },
    {
      day: t("events.attend.day2_title"),
      bg: "bg-[#1E88E5]/10",
      content: t("events.attend.day2_desc")
    }
  ];
  ...
  const whoList = t("events.attend.who_list", { returnObjects: true }) as string[];

  return (
    <div>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <h2 className="text-[32px]  font-medium text-black text-center mb-8">{t("events.attend.title")}</h2>

        <div className="grid md:grid-cols-2 gap-8">
          {/* Top Row: What To Expect */}
          {dayData.map((item, index) => (
            <div key={index} className={`${item.bg} rounded-xl w-full p-6 flex flex-col items-center shadow-sm`}>
              <h3 className="text-[32px] font-medium text-black mb-5">{t("events.attend.expect_title")}</h3>
              <p className="text-2xl font-medium mb-12.5">{item.day}</p>
              <div className="bg-white rounded-2xl px-4 sm:px-6 text-base sm:text-xl font-normal text-black py-3 shadow-inner">
                {item.content}
              </div>
            </div>
          ))}
          ...
          <div className="border border-gray-100 rounded-xl p-6 flex flex-col justify-between shadow-sm">
            <div>
              <h3 className="text-[32px] font-medium text-black mb-8">{t("events.attend.who_title")}</h3>
              <ul className=" text-xl font-normal text-black/60 list-disc list-inside marker:text-black/60 px-2">
                {Array.isArray(whoList) && whoList.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6  relative overflow-hidden">
          <div className="flex flex-col md:flex-row  items-center md:items-start gap-8">
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-40 h-40 sm:w-68 sm:h-65 rounded-xl overflow-hidden">
                <img
